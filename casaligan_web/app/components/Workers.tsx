...
const Workers: React.FC = () => {
  ...
  const [workersActivityData, setWorkersActivityData] = useState<any[]>([]);
  ...
  useEffect(() => {
    async function fetchAnalytics() {
      try {
        const analytics = await getWorkerAnalytics(accountsStartDate || undefined, accountsEndDate || undefined);
        const activityStats = await getWorkerActivityStats(activityStartDate || undefined, activityEndDate || undefined);
        
        setActivityData(analytics.activityData);
        setRestrictedData(analytics.restrictedData);
        setTopWorkers(analytics.topWorkers);
        setNewAccountsData(analytics.newAccountsData);
        setWorkersActivityData(activityStats);
      } catch (error) {
